...
import { Bot, User, Send, Copy, Loader2, Sparkles, File, Upload, Zap } from 'lucide-react'
...
export function ChatPanel() {
  const { fileTree, chatHistory, isAiLoading, quickActions, sendMessage, runQuickAction, codeContext, currentUser } = useEditorStore();
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    sendMessage(message, attachCode, selectedProvider);
    setMessage('');
  }
    ...
    <div className="flex flex-col h-full bg-secondary/30 dark:bg-card">
      <div className="flex items-center justify-between p-2 border-b">
        ...
        <div className="flex items-center gap-2">
          ...
          <Badge variant="outline" className="text-green-500 border-green-500 text-xs">Online</Badge>
        </div>
      </div>
