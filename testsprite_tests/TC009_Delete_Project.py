...
async def run_test():
    ...
    try:
        ...
        # -> Input email and password, then click Sign In button to login and navigate to dashboard.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Project Deletion Successful').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: The project was not deleted successfully from the dashboard and persistent storage as per the test plan.')
