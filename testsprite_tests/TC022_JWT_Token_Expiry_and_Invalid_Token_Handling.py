...
async def run_test():
    ...
    try:
        ...
        # -> Click on 'Sign In' to start login and obtain JWT token
        ...
        # -> Input email and password and click Sign In to login and obtain JWT token
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Access Granted').first).to_be_visible(timeout=30000)
        except AssertionError:
            raise AssertionError('Test failed: Access was not denied for expired or invalid JWT token. User should be redirected to login with an error about session expiration.')
