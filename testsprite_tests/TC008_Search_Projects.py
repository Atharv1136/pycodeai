...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=No Matching Projects Found').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: The project search input did not filter the displayed projects correctly by partial and full matches, case-insensitive as per the test plan.")
