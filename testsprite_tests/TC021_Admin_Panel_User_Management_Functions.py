...
async def run_test():
    ...
    try:
        ...
        # -> Click on 'Sign In' to navigate to the login page for admin credentials input.
        ...
        # -> Input admin email and password, then click the 'Sign In' button to attempt login.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Admin login successful').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Admin user login, user list management, and system statistics verification did not complete successfully as per the test plan.")
